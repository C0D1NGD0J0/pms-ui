...
describe("useLoadingManager", () => {
  ...
  it("should set authenticating state", () => {
    ...
    expect(mockSetLoadingState).toHaveBeenCalledWith(LoadingReason.AUTHENTICATING);
  ...
  it("should set refreshing token state", () => {
    ...
    expect(mockSetLoadingState).toHaveBeenCalledWith(LoadingReason.REFRESHING_TOKEN);
  ...
  it("should set fetching user state", () => {
    ...
    expect(mockSetLoadingState).toHaveBeenCalledWith(LoadingReason.FETCHING_USER);
  });
  ...
  it("should set processing invite state", () => {
    ...
    expect(mockSetLoadingState).toHaveBeenCalledWith(LoadingReason.PROCESSING_INVITE);
  });
  ...
  it("should set idle session state", () => {
    ...
    expect(mockSetLoadingState).toHaveBeenCalledWith(LoadingReason.IDLE_SESSION);
  });
  ...
  it("should set initializing state", () => {
    ...
    expect(mockSetLoadingState).toHaveBeenCalledWith(LoadingReason.INITIALIZING);
  });
  ...
  it("should return current auth state values", () => {
    ...
    expect(result.current.currentLoadingState).toBe(LoadingReason.AUTHENTICATING);
  ...
  it("should maintain stable function references", () => {
    ...
    expect(firstFunctions.setAuthenticating).toBe(secondFunctions.setAuthenticating);
    expect(firstFunctions.setRefreshingToken).toBe(secondFunctions.setRefreshingToken);
    expect(firstFunctions.clearLoadingState).toBe(secondFunctions.clearLoadingState);
  });
  ...
  it("should handle multiple loading state changes", () => {
    ...
    expect(mockSetLoadingState).toHaveBeenNthCalledWith(1, LoadingReason.AUTHENTICATING);
    expect(mockSetLoadingState).toHaveBeenNthCalledWith(2, LoadingReason.REFRESHING_TOKEN);
    expect(mockSetLoadingState).toHaveBeenNthCalledWith(3, LoadingReason.FETCHING_USER);
  });
  ...
  it("should handle boolean toggles for all loading states", () => {
    ...
    const loadingStates = [
      { setter: result.current.setAuthenticating, reason: LoadingReason.AUTHENTICATING },
      { setter: result.current.setRefreshingToken, reason: LoadingReason.REFRESHING_TOKEN },
      { setter: result.current.setFetchingUser, reason: LoadingReason.FETCHING_USER },
      { setter: result.current.setProcessingInvite, reason: LoadingReason.PROCESSING_INVITE },
      { setter: result.current.setIdleSession, reason: LoadingReason.IDLE_SESSION },
      { setter: result.current.setLoggingOut, reason: LoadingReason.LOGGING_OUT },
      { setter: result.current.setInitializing, reason: LoadingReason.INITIALIZING },
    ];
